...
public final class SplashScreen {
    ...
    public static void drawSplash(final TextureManager tm) {
        ...
        if (splash == null) {
            splash = new ResourceLocation("foodtower/wallpaper.png");
        }
    ...
    private static void drawProgress() {
        ...
        String text = "Welcome to FoodTower!";

        // Render the rise text
        final float width = FontManager.F22.getStringWidth(text);
        final float height = FontManager.F22.getStringHeight(text);

        final float x = (sr.getScaledWidth() / 2.0F) - (width / 2.0F);
        final float y = (sr.getScaledHeight() / 2.0F) - (height / 2.0F) - 5;

        FontManager.F22.drawString(text, x, y, new Color(255, 255, 255).getRGB());
    }
}
