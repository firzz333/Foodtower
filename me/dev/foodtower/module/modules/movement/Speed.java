    ...
    private void onUpdate(EventPreUpdate e) {
        if (mode.getValue() == SpeedMode.Hypixel) {
            ...
            if (mc.thePlayer.onGround && MoveUtils.isMoving()) {
                ...
                mc.thePlayer.motionX *= 2.3;
                mc.thePlayer.motionZ *= 2.3;
                if (mc.thePlayer.isPotionActive(Potion.moveSpeed)) {
                    mc.thePlayer.motionX *= (1.0 + 0.3 * (mc.thePlayer.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1));
                    mc.thePlayer.motionZ *= (1.0 + 0.3 * (mc.thePlayer.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1));
                }
            }
        }
    }
